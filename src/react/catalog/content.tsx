...
import { __experimentalHeading as Heading, __experimentalText as WPText, Icon as WPIcon } from '@wordpress/components';
...
export function Text( { props }: A2UIComponentProps< TextProps > ) {
	...
	if ( variant in HEADING_LEVELS ) {
		return (
			<Heading className="a2ui-wp-text" level={ HEADING_LEVELS[ variant as keyof typeof HEADING_LEVELS ] }>
				{ text }
			</Heading>
		);
	}
	if ( variant === 'caption' ) {
		return (
			<WPText className="a2ui-wp-text is-caption" variant="muted" size="small" style={ { whiteSpace: 'pre-wrap' } }>
				{ text }
			</WPText>
		);
	}
...
const IMAGE_VARIANTS: Record< NonNullable< ImageProps[ 'variant' ] >, CSSProperties > = {
	icon: { width: 24, height: 24 },
	avatar: { width: 48, height: 48, borderRadius: '50%' },
	smallFeature: { width: 120, height: 120, borderRadius: 4 },
	mediumFeature: { width: '100%', maxWidth: 320, height: 'auto', borderRadius: 4 },
	largeFeature: { width: '100%', maxWidth: 640, height: 'auto', borderRadius: 4 },
...
export function Image( { props }: A2UIComponentProps< ImageProps > ) {
	...
	const fit = props.fit === 'scaleDown' ? 'scale-down' : props.fit ?? 'fill';
	return (
		<img
			className={ `a2ui-wp-image is-${ props.variant ?? 'mediumFeature' }` }
			src={ url }
			alt={ a11y.label ?? description }
			style={ { objectFit: fit, display: 'block', ...IMAGE_VARIANTS[ props.variant ?? 'mediumFeature' ] } }
		/>
	);
}
...
export function Icon( { props }: A2UIComponentProps< IconProps > ) {
	...
	if ( resolved && typeof resolved === 'object' && typeof ( resolved as { svgPath?: unknown } ).svgPath === 'string' ) {
		const icon = (
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" aria-hidden="true">
				<path d={ ( resolved as { svgPath: string } ).svgPath } fill="currentColor" />
			</svg>
		);
	...
	if ( ! icon ) {
		return <span className="a2ui-wp-icon is-unknown" role="img" aria-label={ a11y.label ?? name } />;
	}
	return (
		<span className="a2ui-wp-icon" role="img" aria-label={ a11y.label ?? name } title={ a11y.description }>
			<WPIcon icon={ icon } />
		</span>
	);
}
...
export function Video( { props }: A2UIComponentProps< VideoProps > ) {
	const url = useDynamicString( props.url );
	return <video className="a2ui-wp-video" src={ url } controls style={ { maxWidth: '100%' } } />;
}
...
export function AudioPlayer( { props }: A2UIComponentProps< AudioPlayerProps > ) {
	const url = useDynamicString( props.url );
	const description = useDynamicString( props.description ?? '' );
	return (
		<div className="a2ui-wp-audio">
			{ description ? <WPText as="p">{ description }</WPText> : null }
			<audio src={ url } controls style={ { width: '100%' } } aria-label={ description || undefined } />
		</div>
	);
}
